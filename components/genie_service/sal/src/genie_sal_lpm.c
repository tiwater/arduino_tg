...
#elif defined(BOARD_TG7121B_EVB)
   return io_read_pin(port) > 0 ? true : false;
#endif
}
...
#elif defined(BOARD_TG7121B_EVB)
   tg7121b_io_wakeup_handler_register(cb);
#endif
...
#elif defined(BOARD_TG7121B_EVB)
   tg7121b_io_wakeup_handler_unregister();
#endif
...
#elif defined(BOARD_TG7121B_EVB)
   return tg7121b_get_bootup_reason();
#endif
...
int32_t genie_sal_lpm_set_bootup_reason(uint32_t reason)
{
   ...
   *(volatile uint32_t *)PWR_BOOT_REASON = reason;
   ...
   return 0;
#elif defined(BOARD_TG7121B_EVB)
   return tg7121b_set_bootup_reason(reason);
#endif
