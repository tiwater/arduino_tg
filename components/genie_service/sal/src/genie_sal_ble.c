...
static void mesh_scan_cb(const bt_addr_le_t *addr, int8_t rssi,
...
int genie_sal_ble_get_rssi(uint8_t mac[6], genie_sal_ble_get_rssi_cb get_rssi_callback)
{
    ...
    if (-ETIMEDOUT == aos_sem_wait(&scan_sem, GENIE_HAL_BLE_SCAN_TIMEOUT))
    {
        scan_timeout();
    }
    aos_sem_free(&scan_sem);
...
#else
static uint8_t g_scan_timer_run = 0;
static aos_timer_t g_scan_timer;
static genie_sal_ble_get_rssi_cb get_rssi_cb = NULL;
static uint8_t test_dev_mac[6] = {0};

static void mesh_scan_cb(const bt_mesh_addr_le_t *addr, int8_t rssi,
                         uint8_t adv_type, struct net_buf_simple *buf)
{
    uint8_t mac_addr[6] = {0};
...
static void scan_timer_cb(void *p_timer, void *args)
{
    bt_mesh_scan_stop();
    bt_mesh_scan_enable();
    if (1 == g_scan_timer_run) //not get rssi in 5s
    {
        aos_timer_stop(&g_scan_timer);
        aos_timer_free(&g_scan_timer);
        g_scan_timer_run = 0;
...
int genie_sal_ble_get_rssi(uint8_t mac[6], genie_sal_ble_get_rssi_cb get_rssi_callback)
{
    ...
    memcpy(test_dev_mac, mac, sizeof(test_dev_mac));
    //printf("test_dev_mac: %s\n", bt_hex(test_dev_mac, sizeof(test_dev_mac)));
    get_rssi_cb = get_rssi_callback;

    bt_mesh_scan_stop();
    int ret = bt_mesh_scan_start(&scan_param, mesh_scan_cb);
    if (ret)
    {
        printf("scan err %d\n", ret);
        memset(test_dev_mac, 0, sizeof(test_dev_mac));
    ...
    if (0 == g_scan_timer_run)
    {
        aos_timer_new(&g_scan_timer, scan_timer_cb, NULL, GENIE_HAL_BLE_SCAN_TIMEOUT, 0);
        aos_timer_start(&g_scan_timer);
        g_scan_timer_run = 1;
    }
...
int genie_sal_ble_send_msg(uint8_t element_id, uint8_t *p_data, uint8_t len)
{
    ...
    if ((len >= 4) && (p_data[0] == VENDOR_OP_ATTR_INDICATE || p_data[0] == VENDOR_OP_ATTR_INDICATE_TG))
    {
        ...
        request_msg.app_idx = BT_MESH_KEY_UNUSED;
