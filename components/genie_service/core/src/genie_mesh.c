...
static int report_poweron_callback(void *p_params, transport_result_e result)
{
#ifdef CONFIG_GENIE_OTA
#if !defined(CONFIG_BT_MESH_NPS_OPT)
...
{
    return genie_mesh_report_poweron();
}
...
{
    if (do_mesh_ready_timer.hdl != NULL)
    {
        aos_timer_stop(&do_mesh_ready_timer);
        aos_timer_free(&do_mesh_ready_timer);
    }
    ...
    if (genie_nps_config_cr_enable_get() == 1)
#endif
    {
...
void genie_mesh_ready_checktimer_restart(void)
{
    ...
    bt_rand(&rand, 1);
#ifdef CONFIG_BT_MESH_NPS_OPT
    random_time = GENIE_MESH_INIT_PROVISIONED_DELAY_START_MIN + genie_nps_config_boot_interval_get() * rand / 255;
#else
...
#ifdef CONFIG_BT_MESH_NPS_OPT
            random_time = GENIE_MESH_INIT_PHARSE_II_DELAY_START_MIN + genie_nps_config_boot_interval_get() * rand / 255;
#else
...
static int genie_mesh_lpn_sub(void)
{
    int index = 0;
    struct bt_mesh_elem *p_elem = NULL;

    //Add user burned group addr
    for (index = 0; index < CONFIG_BT_MESH_MODEL_GROUP_COUNT; index++)
    {
        if (((g_sub_list[index] & 0xC000) == 0xC000)) //valid group address
        {
            bt_mesh_lpn_group_add(g_sub_list[index]);
        }
    }

    //Add hard code group addr
    for (index = 0; index < bt_mesh_elem_count(); index++)
    {
        p_elem = bt_mesh_elem_find_by_id(index);
        if (p_elem && ((p_elem->grop_addr & 0xC000) == 0xC000))
        {
            bt_mesh_lpn_group_add(p_elem->grop_addr);
        }
    }
...
static void genie_mesh_lpn_cb(u16_t friend_addr, bool established)
{
    if (BT_MESH_ADDR_IS_UNICAST(friend_addr) && established)
    {
        genie_mesh_lpn_sub();
    }
}
...
#ifdef CONFIG_GENIE_MESH_PORT_YOC
    genie_sal_event_init();
#endif

#ifdef CONFIG_BT_MESH_LOW_POWER
    bt_mesh_lpn_set_cb(genie_mesh_lpn_cb);
#endif
...
{
#ifndef CONFIG_GENIE_MESH_PORT_YOC
