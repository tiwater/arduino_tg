...
static void _ais_server_indicate(uint8_t msg_id, uint8_t cmd, uint8_t *p_msg, uint16_t len)
{
    ...
    if (genie_ais_ctx.p_conn)
    {
        ...
        BT_INFO("ais ind msgid:%d cmd:%02x p_ind:%p len:%d data:%s\r\n", msg_id, cmd, p_indicate, len, bt_hex(&msg, len + 4));
#else
        BT_INFO("ais ind msgid:%d cmd:%02x p_ind:%p len:%d\r\n", msg_id, cmd, p_indicate, len);
#endif
...
void genie_ais_notify(uint8_t msg_id, uint8_t cmd, uint8_t *p_msg, uint16_t len)
{
    ...
    if (genie_ais_ctx.p_conn)
    {
        ...
        {
            BT_INFO("ais notify msgid:%d cmd:%02x len:%d\r\n", msg_id, cmd, len);
        }
#endif
...
static bool _ais_scrt_result(uint8_t msg_id, ais_scrt_result_t *p_scrt_result)
{
    ...
    {
        genie_ais_ctx.state = AIS_STATE_IDLE;
        genie_ais_set_encrypt_status(1);
    }
...
void genie_ais_set_encrypt_status(uint8_t encrypt)
{
    genie_ais_ctx.is_encrypt = encrypt;
}

uint8_t genie_ais_get_encrypt_status(void)
{
    return genie_ais_ctx.is_encrypt;
}
...
int genie_ais_deinit(void)
{
    bt_gatt_service_unregister(&_ais_srv);
    memset(&genie_ais_ctx, 0, sizeof(genie_ais_ctx));
