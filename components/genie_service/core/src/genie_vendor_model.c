...
int genie_model_handle_mesg(genie_transport_model_param_t *p_msg)
{
    ...
    if (genie_lpm_handle_model_mesg(p_msg) != 0)
    {
        return 0; //This is genie lpm message
    }
#endif
                                 ...
                                 u8_t opid)
{
    ...
    {
        if (opid != VENDOR_OP_ATTR_SET_ACK) //If VENDOR_OP_ATTR_SET_ACK must handle repeat TID mesg
        {
    ...
    msg.p_model = p_model;
    msg.src_addr = p_ctx->addr;
    msg.p_elem = bt_mesh_elem_find_by_id(p_elem->element_id);
