...
uint8_t sig_model_transition_update(sig_model_element_state_t *p_elem)
{
    ...
    if (cycle == 0)
    {
        ...
        GENIE_LOG_INFO("scene_number %02x->%02x", p_state->scene_number[TYPE_PRESENT], p_state->scene_number[TYPE_TARGET]);
        if (p_state->scene_number[TYPE_PRESENT] != p_state->scene_number[TYPE_TARGET])
        {
            p_state->scene_number[TYPE_PRESENT] = p_state->scene_number[TYPE_TARGET];
...
uint8_t sig_model_transition_get_remain_time_byte(sig_model_state_t *p_state, bool is_ack)
{
    ...
    {
        passed_time = cur_time - p_state->trans_start_time;
