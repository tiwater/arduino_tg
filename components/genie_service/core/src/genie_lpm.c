...
#define GENIE_WAKE_UP_IO_NUM_MAX 6
#endif
...
static void genie_lpm_glp_timer_irq_handler(void *p_timer, void *args)
{
    ...
    {
        GENIE_LOG_DBG("lpm already disable, stop timer\n");
    ...
    {
        GENIE_LOG_DBG("[%u]sleep and suspend mesh stack\n", k_uptime_get_32());
...
static void genie_lpm_lpn_power_cb(bool on_off)
{
    if (on_off) //wakeup
    {
        genie_lpm_disable();
...
#ifdef CONFIG_BT_MESH_LOW_POWER
    bt_mesh_lpn_set_power_cb(genie_lpm_lpn_power_cb);
#endif
