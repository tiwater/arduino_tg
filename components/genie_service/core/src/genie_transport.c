...
static void send_opdata_end_cb(int err, void *cb_data)
{
    if (cb_data)
    {
        transport_result_cb result_cb = (transport_result_cb)cb_data;
        result_cb(NULL, SEND_RESULT_DONE);
    }
}
...
static int16_t genie_transport_model_send(genie_transport_model_param_t *p_model_msg)
{
    ...
    if (p_model_msg->opid == SIG_MODEL_OP_DATA_FLAG)
    {
        p_model = p_model_msg->p_model;
        if (!p_model)
        {
            BT_ERR("cannot find sig model server %p\n", p_model_msg->p_elem);
            return err;
        }

        BT_DBG("p_model:%p, app_idx:%d, addr:0x%04x", p_model, p_model_msg->app_idx, p_model_msg->dst_addr);
        BT_DBG("p_model_msg->data:%p, %d, %s", p_model_msg, p_model_msg->len, bt_hex(p_model_msg->data, p_model_msg->len));

        //prepare buffer
        net_buf_simple_init(p_msg, 0);
        if (p_model_msg->len)
        {
        ...
        if (p_model_msg->app_idx != BT_MESH_KEY_UNUSED)
        {
            ctx.app_idx = p_model_msg->app_idx;
        }
        else
        {
    ...
    {
        p_model = bt_mesh_model_find_vnd(p_model_msg->p_elem, CONFIG_MESH_VENDOR_COMPANY_ID, CONFIG_MESH_VENDOR_MODEL_SRV);
        if (!p_model)
        {
    ...
    if (p_model_msg->ttl > 0)
    {
        ctx.send_ttl = p_model_msg->ttl;
    }
    else
    {
    ...
    if (p_model_msg->opid == SIG_MODEL_OP_DATA_FLAG)
    {
        static struct bt_mesh_send_cb model_send_cb = {
            .start = NULL,
            .end = send_opdata_end_cb,
        };
        if (p_model_msg->xmit == BT_MESH_ADV_XMIT_FLAG)
        {
            err = bt_mesh_model_send_ext(p_model, &ctx, p_msg, &model_send_cb, p_model_msg->result_cb, p_model_msg->xmit);
        }
        else
        {
            err = bt_mesh_model_send(p_model, &ctx, p_msg, &model_send_cb, p_model_msg->result_cb);
        }
    }
    else
    {
        if (p_model_msg->xmit == BT_MESH_ADV_XMIT_FLAG)
        {
            err = bt_mesh_model_send_ext(p_model, &ctx, p_msg, NULL, NULL, p_model_msg->xmit);
        }
        else
        {
...
int genie_transport_send_model(genie_transport_model_param_t *p_model_msg)
{
     ...
     * */
        if (((p_model_msg->opid == VENDOR_OP_ATTR_INDICATE) || (p_model_msg->opid == VENDOR_OP_ATTR_TRANS_INDICATE)) && (p_model_msg->tid < GENIE_TRANSPORT_TID_MIN || p_model_msg->tid > GENIE_TRANSPORT_TID_MAX))
        {
            BT_WARN("tid:0x%02x out of range", p_model_msg->tid);
            break;
        }
...
int genie_transport_send_payload(genie_transport_payload_param_t *payload_param)
{
    ...
    transport_model_param.p_elem = bt_mesh_elem_find_by_id(payload_param->element_id);
...
int genie_transport_find_by_opcode(uint16_t opcode)
{
    sys_dnode_t *p_node = NULL;
    genie_transport_model_param_t *p_msg = NULL;
    genie_transport_node_t *p_msg_node = NULL;
    uint16_t node_opcode = 0;
    ...
    SYS_DLIST_FOR_EACH_NODE(&send_list, p_node)
    {
        ...
        p_msg = &p_msg_node->msg;
        if ((p_msg->opid == SIG_MODEL_OP_DATA_FLAG) && (p_msg_node->left_retry > 0))
        {
            node_opcode = (p_msg->data[0] << 8) + p_msg->data[1];
            if (node_opcode == opcode)
            {
                GENIE_LOG_INFO("msg opcode:0x%04x", node_opcode);
                return 1;
            }
        }
    }
...
int genie_transport_remove_by_opid(uint8_t opid)
{
    sys_dnode_t *p_node = NULL;
    sys_dnode_t *p_node_safe = NULL;
    genie_transport_model_param_t *p_msg = NULL;
    ...
    SYS_DLIST_FOR_EACH_NODE_SAFE(&send_list, p_node, p_node_safe)
    {
        ...
        p_msg = &p_msg_node->msg;
        if ((p_msg->opid == opid) && (p_msg_node->left_retry > 0))
        {
            GENIE_LOG_INFO("remove msg opcode:0x%04x", (p_msg->data[0] << 8) + p_msg->data[1]);
    ...
    SYS_DLIST_FOR_EACH_NODE_SAFE(p_head, p_node, p_node_safe)
    {
        ...
        if (p_msg_node->timeout <= k_uptime_get())
        {
            ...
            if (p_msg_node->left_retry-- == 0)
            {
                ...
                {
                    p_msg_node->msg.result_cb(&p_msg_node->msg, SEND_RESULT_TIMEOUT);
                }

                if (p_msg_node->msg.opid != SIG_MODEL_OP_DATA_FLAG)
                {
...
int genie_transport_ack(uint8_t tid)
{
    ...
    SYS_DLIST_FOR_EACH_NODE_SAFE(&send_list, p_node, p_node_safe)
    {
        ...
        if (p_msg->tid == tid)
        {
            ...
            if (p_msg->result_cb)
            {
                p_msg->result_cb(p_msg, SEND_RESULT_SUCCESS);
            }
