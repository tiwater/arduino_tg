...
static genie_event_e _genie_event_handle_pbadv_timeout(void)
{
    ...
    return GENIE_EVT_SDK_MESH_PBADV_STOP; //provision stop
}
...
static genie_event_e genie_model_notify_onoff_msg(genie_time_event_t *time_event) //notify MCU
{
    genie_transport_model_param_t msg;
    uint8_t payload[4] = {0};

    BT_DBG("type:%04x data:%04x\r\n", time_event->attr_data.type, time_event->attr_data.para);

    if (time_event->attr_data.type == ATTR_TYPE_GENERIC_ONOFF)
    {
        payload[0] = time_event->element_id;
        payload[1] = 0x00;
        payload[2] = 0x01;
        payload[3] = time_event->attr_data.para;
...
void genie_event(genie_event_e event, void *p_arg)
{
    ...
    case GENIE_EVT_SDK_APPKEY_ADD:
    {
        ...
        genie_ultra_prov_done(ULTRA_PROV_SUCCESS);
#endif
    ...
    {
        next_event = genie_model_notify_onoff_msg((genie_time_event_t *)p_arg);
    }
    break;
#endif
    default:
