...
#define at_debug aos_cli_printf
...
int genie_at_cmd_parser(char data[])
{
    ...
    ustype = argv_type(data);
    memset(argv, 0, sizeof(argv));
    memset(argqv, 0, sizeof(argqv));
...
int genie_at_cmd_mesh_test(int argc, char *argv[])
{
    ...
    case MESH_TEST_RSSI:
        ...
        err = genie_sal_ble_get_rssi(test_dev_mac, (genie_sal_ble_get_rssi_cb)get_ble_rssi_cb);
        if (err != 0)
        {
