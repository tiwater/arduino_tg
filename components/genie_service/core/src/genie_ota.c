...
void genie_ota_status_report(void)
{
    ...
    GENIE_LOG_INFO("ota last[%d] total[%d] size[%ld] err[%d]", p_status_report->last_seq, p_status_report->total_frame, p_status_report->rx_size, genie_ota_ctx.err_count);

    if (genie_ais_get_encrypt_status())
    {
    ...
    {
        genie_ais_notify(0, AIS_OTA_STATUS, plaine_data, 5);
    }
}
...
bool genie_ota_parse_pdu(ais_pdu_t *p_msg)
{
    ...
    cur_progress = genie_ota_ctx.rx_size * 100 / genie_ota_ctx.image_size;
    if (cur_progress - genie_ota_ctx.progress > 9)
    {
        printf("ota progress:%d\n", cur_progress);
        genie_ota_ctx.progress = cur_progress;
    }

    if (p_msg->header.seq == p_msg->header.total_frame)
    {
        genie_sal_ota_update_image(genie_ota_ctx.image_type,
                                   genie_ota_ctx.rx_size - genie_ota_ctx.rx_len, (uint32_t)genie_ota_ctx.rx_len,
                                   genie_ota_ctx.recv_buf);
        genie_ota_ctx.rx_len = 0;
    ...
    else if (genie_ota_ctx.rx_size == genie_ota_ctx.image_size)
    {
        if (payload_len != 0) //This is last pdu
        {
            genie_ota_status_report();
        }
    }
...
bool genie_ota_check_firmware(uint8_t msg_id, ais_ota_check_req_t *p_check_req)
{
    ...
    if (p_check_req->state == 1)
    {
        ...
        {
            genie_ais_notify(msg_id, AIS_OTA_CHECK_RESP, plaine_data, 1);
        }
...
static int ota_report_version_result_cb(void *p_params, transport_result_e result_e)
{
