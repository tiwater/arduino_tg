...
genie_storage_status_e genie_storage_write_sub(uint16_t *p_sub)
{
    ...
    group_addr_crc = genie_crc16_compute(data, CONFIG_BT_MESH_MODEL_GROUP_COUNT * 2, &crc_magic);
...
genie_storage_status_e genie_storage_write_appkey(uint16_t key_idx, mesh_appkey_para_t *p_appkey)
{
    if (key_idx == 0)
    {
    ...
    {
        return genie_storage_write_reliable(GFI_MESH_APPKEY1, (uint8_t *)p_appkey, sizeof(mesh_appkey_para_t));
    }

    return GENIE_STORAGE_INVALID_INDEX;
}

genie_storage_status_e genie_storage_read_appkey(uint16_t key_idx, mesh_appkey_para_t *p_appkey)
{
    if (key_idx == 0)
    {
    ...
    {
        return genie_storage_read_reliable(GFI_MESH_APPKEY1, (uint8_t *)p_appkey, sizeof(mesh_appkey_para_t));
    }
