...
static int genie_time_periodic_start(uint8_t element_id, uint8_t index, uint32_t periodic_time, uint8_t schedule, vendor_attr_data_t *attr_data)
{
    ...
    vendor_timer->element_id = element_id;
...
static int genie_time_operate_status(uint8_t element_id, uint8_t tid, u16_t attr_type)
{
    u16_t payload_len = 0;
...
static int genie_time_start(uint8_t element_id, uint8_t index, uint32_t unix_time, vendor_attr_data_t *attr_data)
{
    genie_time_t *vendor_timer;
...
int genie_time_timing_settting_event(u8_t element_id, u8_t op, u8_t *msg, u16_t msg_length, uint8_t tid)
{
    struct
    {
        uint32_t unix_time;
    ...
    if (op == VENDOR_OP_ATTR_SET_ACK)
    {
        while (msg_len > 0)
        {
            ...
            int ret = genie_time_start(element_id, timing_setting_attr.index,
...
int genie_time_priordic_timing_settting_event(u8_t element_id, u8_t op, u8_t *msg, u16_t msg_length, uint8_t tid)
{
    struct
    {
        uint8_t index;
    ...
    if (op == VENDOR_OP_ATTR_SET_ACK)
    {
        while (msg_len > 0)
        {
            ...
            int ret = genie_time_periodic_start(element_id, priordic_timing_attr.index,
...
int genie_time_timing_remove_event(u8_t element_id, u8_t op, u8_t *msg, u16_t msg_length, uint8_t tid)
{
    u8_t *pmsg = msg;
    u16_t attr_type = *pmsg++;
    uint16_t msg_len = msg_length;
...
static void genie_time_do_work(struct k_work *work)
{
    ...
    SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&genie_time_timer.timer_list_active, node, tmp, next)
    {
        if (is_genie_time_timeout(node))
        {
            ...
            {
                genie_time_timer.genie_time_event_cb(GT_TIMEOUT, node->element_id, node->index, &node->attr_data);
            }
            ...
            {
                node->unixtime_match += (1 + next_weekday_diff_get(next_weekday(local_time.weekday), node->schedule)) * DAY;
            }
    ...
    if (genie_time_timer.unix_time_sync_match && genie_time_timer.unix_time_sync_match <= genie_time_timer.unix_time)
    {
        if (genie_time_timer.genie_time_event_cb)
        {
            int ret = genie_time_timer.genie_time_event_cb(GT_TIMING_SYNC, 0, 0, NULL);
...
static int genie_time_update(void)
{
    ...
    {
        genie_time_timer.genie_time_event_cb(GT_TIMING_SYNC, 0, 0, NULL);
        return 0;
    }
...
int genie_time_handle_model_mesg(genie_transport_model_param_t *p_msg)
{
    ...
    element_id = bt_mesh_elem_find_id(p_msg->p_elem);
    ...
    case VENDOR_OP_ATTR_GET_STATUS:
    {
        ...
        {
            genie_time_operate_status(element_id, p_msg->tid, attr_type);
        }
        else if (attr_type == TIMING_SETTING_T)
        {
            genie_time_timing_settting_event(element_id, VENDOR_OP_ATTR_GET_STATUS, p_msg->data, p_msg->len, p_msg->tid);
        }
        else if (attr_type == TIMING_PERIODIC_SETTING_T)
        {
            genie_time_priordic_timing_settting_event(element_id, VENDOR_OP_ATTR_GET_STATUS, p_msg->data, p_msg->len, p_msg->tid);
        }
        else
        {
            is_time_mesg = 0;
    ...
    case VENDOR_OP_ATTR_SET_ACK:
    {
        ...
        {
            genie_time_timing_settting_event(element_id, VENDOR_OP_ATTR_SET_ACK, p_msg->data, p_msg->len, p_msg->tid);
        }
        else if (attr_type == TIMING_PERIODIC_SETTING_T)
        {
            genie_time_priordic_timing_settting_event(element_id, VENDOR_OP_ATTR_SET_ACK, p_msg->data, p_msg->len, p_msg->tid);
        }
        else if (attr_type == TIMING_DELETE_T)
        {
            genie_time_timing_remove_event(element_id, VENDOR_OP_ATTR_SET_ACK, p_msg->data, p_msg->len, p_msg->tid);
        }
        else
        {
            is_time_mesg = 0;
...
static int genie_time_event_callback(uint8_t event, uint8_t element_id, uint8_t index, vendor_attr_data_t *data)
{
    if (event == GT_TIMEOUT)
    {
        ...
        genie_time_event.element_id = element_id;
        genie_time_event.attr_data.para = data->para;
        genie_time_event.attr_data.type = data->type;
        genie_event(GENIE_EVT_TIMEOUT, &genie_time_event);
