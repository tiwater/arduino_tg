...
#ifdef MESH_DEBUG_RX
#if defined(BOARD_TG7121B_EVB) || defined(BOARD_TG7130B_EVB)
	printf("\t[RX] RSSI: %d\n", rx->rssi);
	printf("\t[RX] TTL: %d AppIdx: 0x%04x\n", rx->ctx.recv_ttl, rx->ctx.app_idx);
	printf("\t[RX] SRC: 0x%02X\n", rx->ctx.addr);
	printf("\t[RX] DST: 0x%02X\n", rx->ctx.recv_dst);
	printf("\t[RX] OPCODE: 0x%04X\n", opcode);
	printf("\t[RX] Payload size: %d\n", buf->len);
...
#ifdef MESH_DEBUG_TX
#if defined(BOARD_TG7121B_EVB) || defined(BOARD_TG7130B_EVB)
	printf("\t[TX] TTL: %d AppIdx: 0x%04x\n", tx->ctx->send_ttl, tx->ctx->app_idx);
	printf("\t[TX] SRC: 0x%02X\n", bt_mesh_model_elem(model)->addr);
	printf("\t[TX] DST: 0x%02X\n", tx->ctx->addr);
