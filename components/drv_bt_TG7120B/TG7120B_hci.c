...
//extern int drv_enable_irq1(int cs);
//extern int drv_disable_irq1(void);
extern void TIM1_IRQHandler1(void);
...
extern llStatus_t LL_SetAdvControl1( uint8 advMode );
extern void llSetupSecScan1( uint8 chan );
extern llStatus_t LL_SetScanParam1( uint8  scanType,
                             uint16 scanInterval,
                             uint16 scanWindow,
                             uint8  ownAddrType,
                             uint8  scanWlPolicy );
extern llStatus_t LL_SetScanControl1( uint8 scanMode,
                               uint8 filterReports );
extern void llProcessSlaveControlPacket2( llConnState_t *connPtr,
  uint8         *pBuf );
...
void init_config(void)
{
    ...
    pGlobal_config[LL_HW_PLL_DELAY] = 40;   //reduce PLL_DELAY for scanner 11/11
    // Tx2Rx and Rx2Tx interval
    //Tx2Rx could be advanced a little
    //Rx2Tx should be ensure T_IFS within150us+-2us
    pGlobal_config[LL_HW_Rx_TO_TX_INTV] = 63-RF_PHY_EXT_PREAMBLE_US;
    ...
    if (g_system_clk == SYS_CLK_XTAL_16M) {
        ...
        pGlobal_config[SCAN_RSP_DELAY] = 16+RF_PHY_EXT_PREAMBLE_US;//13;//23;
    } else if (g_system_clk == SYS_CLK_DLL_48M) {
        // scan req -> scan rsp timing
        pGlobal_config[SCAN_RSP_DELAY] = 11+RF_PHY_EXT_PREAMBLE_US;//4;        // 12    //  2019/3/19 A2: 12 --> 9
...
//    pGlobal_config[LL_TX_PWR_TO_REG_BIAS]   = 0x15;   // assume when g_rfPhyTxPower = 0x1f, tx power = 10dBm
    ...
    if (g_system_clk == SYS_CLK_XTAL_16M) {
        ...
        pGlobal_config[LL_ADV_TO_CONN_REQ_DELAY]    = 15+RF_PHY_EXT_PREAMBLE_US;//27;      //  2019/3/19 A2: 27 --> 25 //reduce scanner(23) 11/11
    } else if (g_system_clk == SYS_CLK_DLL_48M) {
        pGlobal_config[LL_ADV_TO_SCAN_REQ_DELAY]    = 10+RF_PHY_EXT_PREAMBLE_US;//12;      //  2019/3/19 A2: 12 --> 10
        pGlobal_config[LL_ADV_TO_CONN_REQ_DELAY]    = 10+RF_PHY_EXT_PREAMBLE_US;//18;
...
#else
    pGlobal_config[LL_SEC_SCAN_MARGIN] = 4200; // 1400*3;  to avoid mesh proxy llTrigErr 0x15
#endif
...
void ll_patch_init()
{
    ...
    JUMP_FUNCTION(LL_PROCESS_SLAVE_CTRL_PKT) = (uint32_t)&llProcessSlaveControlPacket2;
