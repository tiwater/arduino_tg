...
{
    uint32_t offset_read = offset + 0x1807f000;
    return hal_flash_read(HAL_ALL_FLASH_0,&offset_read,buffer,length);
}

int dut_hal_factorydata_store(uint32_t offset, uint8_t *buffer, uint32_t length)
{
    uint32_t offset_write = offset + 0x1807f000;
    return hal_flash_write(HAL_ALL_FLASH_0,&offset_write,buffer,length);
}
...
{
    uint32_t offset = 0x100 + 0x1807f000;
    return hal_flash_read(HAL_ALL_FLASH_0,&offset,addr,6);
}

int dut_hal_mac_store(uint8_t addr[6])
{
    uint32_t offset = 0x100 + 0x1807f000;
    return hal_flash_write(HAL_ALL_FLASH_0,&offset,addr,6);
}
...
int dut_hal_xtalcap_set(uint32_t xtalCap)
{
    return 0;
}
