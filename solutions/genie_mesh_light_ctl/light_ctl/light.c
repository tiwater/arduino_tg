...
// load light param
    ...
    /* power on use default value ON for test */
    ret = GENIE_STORAGE_INIT_FAIL;
#else
...
static void light_ctl_handle_order_msg(genie_time_event_t *time_event)
{
    GENIE_LOG_INFO("element_id:%d type:%04x data:%04x\r\n", time_event->element_id, time_event->attr_data.type, time_event->attr_data.para);

    if (time_event->attr_data.type == ATTR_TYPE_GENERIC_ONOFF)
    {
#ifdef CONFIG_MESH_MODEL_GEN_ONOFF_SRV
        light_elem_stat[0].state.onoff[TYPE_TARGET] = time_event->attr_data.para;

        if (light_elem_stat[0].state.onoff[TYPE_PRESENT] != light_elem_stat[0].state.onoff[TYPE_TARGET])
        {
            sig_model_generic_onoff_bind_ops(&light_elem_stat[0]);
...
#endif
#ifdef CONFIG_MESH_MODEL_SCENE_SRV
    if (p_elem->state.scene[TYPE_TARGET] > 0)
    {
...
static void light_ctl_event_handler(genie_event_e event, void *p_arg)
{
    ...
    {
#ifdef CONFIG_BT_MESH_NPS_OPT
        GENIE_LOG_INFO("User report poweron status");
        sig_model_report_poweron_status(&light_elem_stat[0]);
#else
    ...
    {
        genie_time_event_t *p_genie_time_event = (genie_time_event_t *)p_arg;
        //User handle vendor timeout event at here
        if (p_genie_time_event)
        {
            light_ctl_handle_order_msg(p_genie_time_event);
        }
    }
    break;
#endif
